...
begin
 Label1.Caption:='Дан текст. Удалить слова, у которых нет повторяющихся букв.';
end;

if (ComboBox1.ItemIndex=1) then
begin
 Label1.Caption:='В тексте все слова начинаются с различных букв. Построить цепочку слов предложения в таком порядке, чтобы последняя буква каждого слова совпадала с первой буквой следующего слова.';
end;
end;
...
setlength(NewStrAr,CountWord);
    ...
    while i <= len do
        if (((lowercase(TwoTaskText[i]) >= 'a')) and (lowercase(TwoTaskText[i]) <= 'z')) or (((lowercase(TwoTaskText[i]) >= 'A')) and (lowercase(TwoTaskText[i]) <= 'Z')) or (((lowercase(TwoTaskText[i]) >= 'а')) and (lowercase(TwoTaskText[i]) <= 'я')) or (((lowercase(TwoTaskText[i]) >= 'А')) and (lowercase(TwoTaskText[i]) <= 'Я')) then begin
            Word := lowercase(TwoTaskText[i]);
            i := i + 1;
            while (i <= len) and (((lowercase(TwoTaskText[i]) >= 'a')) and (lowercase(TwoTaskText[i]) <= 'z')) or (((lowercase(TwoTaskText[i]) >= 'A')) and (lowercase(TwoTaskText[i]) <= 'Z')) or (((lowercase(TwoTaskText[i]) >= 'а')) and (lowercase(TwoTaskText[i]) <= 'я')) or (((lowercase(TwoTaskText[i]) >= 'А')) and (lowercase(TwoTaskText[i]) <= 'Я')) do begin
